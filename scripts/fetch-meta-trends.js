...
const MAX_ADS_PER_QUERY = Number(process.env.META_MAX_ADS_PER_QUERY || 10);
...
// Keep the first run small so Apify does not abort.
const queries = [
  "cat litter box",
  "pet products",
  "beauty tools",
...
async function apify(path, options = {}) {
  ...
  if (!res.ok) {
    throw new Error(`Apify error ${res.status}: ${JSON.stringify(data).slice(0, 700)}`);
  }
...
async function runActor() {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (["FAILED", "ABORTED", "TIMED-OUT"].includes(runStatus)) {
      const details = status?.data?.statusMessage || status?.data?.exitCode || "";
      throw new Error(`Apify run ended with status ${runStatus}. ${details}`);
    }
...
function keywordsForAd(ad) {
  const text = [ad.pageName, ad.adCopy, ad.headline, ad.landingUrl]
    .map(cleanText)
    .join(" ")
    .toLowerCase();
...
async function main() {
  ...
  const ads = Array.isArray(items)
    ? items.map(normalizeAd).filter(a => a.pageName || a.adCopy || a.headline)
    : [];
