...
function normalizeProduct(p) {
  ...
  // Public CJ catalog usually supports listing/listed style signals.
  // Sales volume, wishlist, and growth are not guaranteed in product-list response.
  const listedCount = number(first(
    p.listedNum, p.listedCount, p.listingCount, p.listNum, p.productListedNum
  ));

  const inventory = number(first(
    p.inventory, p.totalInventory, p.stock, p.stockNum, p.productStock, p.availableQuantity
  ));

  // Keep these only if CJ returns them in your account/API response.
  const orders = number(first(p.productSellNum, p.sellNum, p.orderNum, p.orders, p.sales, p.saleNum));
  const cjRank = number(first(p.rank, p.bestsellerRank, p.categoryRank, 0));
...
function demandScore(p) {
  ...
  const inventory = Math.min(20, Math.log10(number(p.inventory) + 1) * 7);
  ...
  return listed + inventory + orders + rank;
}
...
async function fetchProductsPage(token, pageNum) {
  // searchType=2 requests CJ Trending Products where supported by CJ.
  // orderBy=listedNum + sort=desc prioritizes widely listed/synced products.
...
async function main() {
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    console.log(`Fetching CJ trending page ${page}/${MAX_PAGES}...`);
    ...
    if (!sampleRaw) sampleRaw = items[0];
  ...
  fs.writeFileSync("products-meta.json", JSON.stringify({
    ...
    mode: "CJ Trending Products",
    query: {
      searchType: 2,
      orderBy: "listedNum",
      sort: "desc"
    },
    ranking: "winningScore uses CJ-supported listing count, inventory if provided, orders if provided, valid pricing, profit, margin, image, and supplier quality.",
    note: "CJ product list does not always provide public sales volume, wishlist/favorites, recent growth, or bestseller rank."
  }, null, 2));
  ...
  console.log(`Saved ${all.length} CJ trending products to products.json`);
}
